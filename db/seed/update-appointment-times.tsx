...
export async function updateAppointmentsForUser(userId: string) {
  ...
  for (const patient of patients) {
    ...
    for (const appointment of appointments) {
      ...
      // Parse the appointment's seeded date.
      const appointmentDate = moment(appointment.date)

      // Earliest start: 8:00 AM on the appointment's date.
      const earliestStart = appointmentDate
        ...
        .second(0)
        .millisecond(0)
      // Latest start: 5:00 PM on that day minus the chosen duration.
      ...
      if (currentTime.minute() % 30 !== 0) {
        // If before 30, set minutes to 30; if after, jump to the next hour.
